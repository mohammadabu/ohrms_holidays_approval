...
class HrLeave(models.Model):
    ...
    def add_validators(self):
        ...
        #     record_set = self.env['hr.holidays.validators'].search([('hr_holiday_status','=',5)])
        #     record_set.unlink()
        #     emp_positions = self.env['hr.job'].sudo().search([('internal_id','=','HR Manager')])
        #     for emp_position in emp_positions: 
        #         employees = self.env['hr.employee'].sudo().search([('multi_job_id','in',emp_position.id)])
        #         for employee in employees:
        #             if employee.user_id != False:
        #                 joker = {'hr_holiday_status':5, 
        #                         'holiday_validators':employee.user_id.id,
        #                         }
        #                 self.env['hr.holidays.validators'].create(joker)
        li = []
